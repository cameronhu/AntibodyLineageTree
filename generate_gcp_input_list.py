...
def list_samples_gcs(input_dir, output_path):
    ...
    Args:
        input_dir (str): GCS directory path (e.g., 'proevo-ab/oas/unpaired/unpaired_human/unpaired_human_heavy').
        output_path (str): GCS path for the output TSV file (e.g., 'gs://proevo-ab/lineages/fastbcr/batch/run_to_files.tsv').
    """
    ...
    for run_id, files in run_to_files.items():
        tsv_lines.append(f"{run_id}\t{','.join(files)}")
    tsv_content = "\n".join(tsv_lines)
